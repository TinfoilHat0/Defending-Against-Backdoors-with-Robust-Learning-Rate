...
class Aggregation():
    ...
    def aggregate_updates(self, global_model, agent_updates_dict, cur_round):
        ...
        if self.args.robust_lr > 0:
            lr_vector = self.compute_robustLR(agent_updates_dict, cur_round)
    ...
    def compute_robustLR(self, agent_updates_dict, cur_round):
        agent_updates_sign = [torch.sign(update) for update in agent_updates_dict.values()]  
        sm_of_signs, adjusted_lr = torch.abs(sum(agent_updates_sign)), 0
        threshold = self.args.robustLR_threshold
        
        if self.args.robust_lr == 1:
            sm_of_signs[sm_of_signs < threshold] = 0 
            sm_of_signs[sm_of_signs >= threshold] = self.server_lr
            adjusted_lr = sm_of_signs
               
        elif self.args.robust_lr == 2:
            sm_of_signs[sm_of_signs < threshold] = -self.server_lr
            sm_of_signs[sm_of_signs >= threshold] = self.server_lr
            adjusted_lr = sm_of_signs                                                
        return adjusted_lr.to(self.args.device)
